...
window.Store = (function() {
    ...
    function init() {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', function() {
                _syncSearchClear();
                _queueSuggest(searchInput.value);
            });
            searchInput.addEventListener('keydown', function(e) {
                ...
                if (e.key === 'Enter') {
                    e.preventDefault();
                    if (_suggestActive >= 0 && _suggestGames[_suggestActive]) {
                        _openSuggest(_suggestGames[_suggestActive]);
                    } else {
                        _hideSuggest();
            ...
            searchInput.addEventListener('focus', function() {
                if (String(searchInput.value || '').trim()) _queueSuggest(searchInput.value);
            });
        }

        var searchClear = document.getElementById('store-search-clear');
        if (searchClear) {
        ...
        document.addEventListener('click', function(e) {
            if (!e.target.closest('.store-search-wrap')) _hideSuggest();
        });
        ...
        document.querySelectorAll('.store-top-tab').forEach(function(tab) {
            tab.addEventListener('click', function() {
                document.querySelectorAll('.store-top-tab').forEach(function(t) { t.classList.remove('active'); });
                tab.classList.add('active');
                _sortBy = tab.dataset.sort || 'newest';
                var sortSelect = document.getElementById('store-sort');
                if (sortSelect) sortSelect.value = _sortBy;
                var sortUi = document.querySelector('#store-sort-ui .custom-select-text');
                if (sortUi && sortSelect && sortSelect.options[sortSelect.selectedIndex]) {
                    sortUi.textContent = sortSelect.options[sortSelect.selectedIndex].text;
                }
                _page = 1;
                _fetchGames();
            });
        ...
        document.addEventListener('click', function(e) {
            ...
            var card = e.target.closest('#store-grid .game-card, #store-list .game-list-item, #store-list .steam-row');
            if (!card) return;
            var appId = card.dataset.appid;
            if (!appId) return;
            e.preventDefault();
            e.stopPropagation();
            var nameEl = card.querySelector('.game-card-name, .game-list-name, .steam-row-name');
        ...
        Bridge.on('search_results', function(json) {
            ...
            try {
                ...
                if (data.request_id && String(data.request_id).indexOf('store-suggest-') === 0) {
                    if (String(data.request_id) !== 'store-suggest-' + _suggestSeq) return;
                    var hints = data.games || [];
                    if (_blockNsfw) {
                        hints = hints.filter(function(g) { return !g.nsfw && !_looksNsfwByName(g); });
                    }
                    _renderSuggest(hints);
                    return;
                }
                if (data.request_id && String(data.request_id).indexOf('store-prefetch-') === 0) {
                    var p = parseInt(String(data.request_id).slice('store-prefetch-'.length), 10);
                    if (p) {
                        _prefetch = {
                            page: p,
                            key: _prefetchKey(),
                ...
                _hideLoading();
                if (data.error) {
                    Components.showToast('error', Components.friendlyError(data.error));
                }
                ...
                _hideLoading();
                Components.showToast('error', Components.friendlyError('Failed to parse search results'));
            }
        });
    ...
    function _syncSearchClear() {
        ...
        var clear = document.getElementById('store-search-clear');
        if (!clear) return;
        var has = !!(input && String(input.value || '').trim());
        clear.classList.toggle('hidden', !has);
    }

    function _queueSuggest(raw) {
    ...
    function _hideSuggest() {
        _suggestActive = -1;
        _suggestGames = [];
        var box = document.getElementById('store-suggest');
        if (box) {
            box.innerHTML = '';
    ...
    function _moveSuggest(delta) {
        if (!_suggestGames.length) return;
        _suggestActive = (_suggestActive + delta + _suggestGames.length) % _suggestGames.length;
        var box = document.getElementById('store-suggest');
        if (!box) return;
        var items = box.querySelectorAll('.store-suggest-item');
        items.forEach(function(el, i) {
            el.classList.toggle('is-active', i === _suggestActive);
        });
        if (items[_suggestActive] && items[_suggestActive].scrollIntoView) {
            items[_suggestActive].scrollIntoView({ block: 'nearest' });
        }
    }

    function _openSuggest(game) {
        if (!game) return;
        _hideSuggest();
        if (window.GameDetails && game.app_id) {
            GameDetails.show(game.app_id, game.name || '');
        }
    }

    function _renderSuggest(games) {
        var box = document.getElementById('store-suggest');
        var input = document.getElementById('store-search');
        if (!box || !input || !String(input.value || '').trim()) {
            _hideSuggest();
            return;
        }
        _suggestGames = (games || []).slice(0, 8);
        _suggestActive = _suggestGames.length ? 0 : -1;
        if (!_suggestGames.length) {
            box.innerHTML = '<div class="store-suggest-label">Search results</div>' +
                '<div class="store-suggest-empty">No matching titles</div>';
    ...
    function _fetchGames() {
        ...
        if (_prefetch && _prefetch.page === _page && _prefetch.key === key) {
            var cached = _prefetch;
            _prefetch = null;
            _activeRequestId = 'from-prefetch';
            _initialFetchDone = true;
            _hideLoading();
            var cachedGames = cached.games || [];
            if (_blockNsfw) {
                cachedGames = cachedGames.filter(function(g) { return !g.nsfw && !_looksNsfwByName(g); });
            }
            _renderGames(cachedGames);
            _total = cached.total || cachedGames.length;
            _totalPages = Math.max(1, Math.ceil(_total / _perPage));
            _updatePagination();
            _schedulePrefetch();
            return;
    ...
    function _prefetchKey() {
        return [_searchQuery || '', _sortBy || '', _activeGenre || '', String(_perPage)].join('\0');
    }
    ...
    function _schedulePrefetch() {
        ...
        var fromPage = _page;
        var key = _prefetchKey();
        var nextPage = fromPage + 1;
        _prefetchTimer = setTimeout(function() {
            if (!_active || _page !== fromPage || _prefetchKey() !== key) return;
            Bridge.call('search_games', _searchQuery, fromPage * _perPage, _perPage, _sortBy, _activeGenre, 'store-prefetch-' + nextPage);
        }, 1400);
    }

    function _showCatalogBanner() {
        Bridge.callWithCallback('get_catalog_status', function(json) {
            var data = {};
            try { data = JSON.parse(json || '{}'); } catch (e) {}
            var el = document.getElementById('store-catalog-banner');
            if (!el) return;
            if (data.stale) {
                el.classList.remove('hidden');
                el.textContent = data.loaded
                    ? 'Game catalog is stale. Click Update List when you are online.'
                    : 'Catalog is empty or offline. Click Update List to download the game list.';
            } else {
    ...
    function _releaseStoreImages() {
        document.querySelectorAll('#store-grid img, #store-list img').forEach(function(img) {
            if (window.Components && Components.releaseCoverImg) {
                Components.releaseCoverImg(img);
            } else {
                img.onload = null;
                img.onerror = null;
    ...
    function _renderGames(games) {
        ...
        if (games.length === 0) {
            var emptyMsg = _searchQuery
                ? 'No games match this search. Try another name or App ID.'
                : 'No games in the catalog. Click Update List when you are online.';
            var cta = _searchQuery
                ? '<button type="button" class="btn" id="store-empty-cta">Clear search</button>'
                : '<button type="button" class="btn btn-primary" id="store-empty-cta">Update List</button>';
            var empty = '<div class="empty-state"><p>' + emptyMsg + '</p>' + cta + '</div>';
            if (_viewMode === 'list') {
                if (list) list.innerHTML = empty;
            } else {
                if (grid) grid.innerHTML = empty;
            }
            var ctaBtn = document.getElementById('store-empty-cta');
            if (ctaBtn) {
                ctaBtn.addEventListener('click', function() {
                    if (_searchQuery) {
                        ...
                        var upd = document.getElementById('store-update-list-btn');
                        if (upd) upd.click();
                    }
                });
            }
    ...
    function _renderCurrentView() {
        ...
        if (_viewMode === 'list') {
            ...
            _lastGames.forEach(function(game) {
                if (Components.createSteamRow) {
                    fragment.appendChild(Components.createSteamRow(game));
                } else {
    ...
    function _applySelectionState() {
        ...
        document.querySelectorAll('#store-grid .game-card, #store-list .game-list-item, #store-list .steam-row').forEach(function(card) {
