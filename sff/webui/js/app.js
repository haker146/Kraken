...
window.App = (function() {
    ...
    function init() {
        ...
        Bridge.onReady(function(py) {
            ...
            py.get_platform(function(platform) {
                ...
                if (_currentPage === 'tools') _refreshHomeLumacoreNotice();
                ...
                if (_platform !== 'win32') {
                    Bridge.callWithCallback('get_setting', 'linux_guide_shown', function(val) {
                        if (val === 'True') return;
                        setTimeout(function() {
                            var msg = 'Kraken has been installed on Linux.\n\n';
            ...
            py.get_setting('theme', function(themeId) {
                ...
                localStorage.setItem('theme', 'steam');
                document.body.style.backgroundImage = '';
            ...
            Bridge.on('task_finished', function(json) {
                try {
                    ...
                    if (result.task === 'auto_lc_setup') {
                        ...
                        if (_currentPage === 'tools') _refreshHomeLumacoreNotice(true);
                    }
            ...
            Bridge.on('log_message', function(msg) {
                ...
                var updateHomeLog = (_currentPage === 'tools');
    ...
    function navigateTo(pageId) {
        ...
        window.requestAnimationFrame(function() {
            ...
            switch(pageId) {
                case 'home':
                    if (window.Home && Home.onPageEnter) Home.onPageEnter();
                    break;
                case 'tools':
                    _populateGameDropdown();
        ...
        if (deactivateBtn) {
            deactivateBtn.addEventListener('click', function() {
                var ok = window.confirm(
                    'Deactivate LumaCore?\n\n' +
                    'Steam will be closed first. Kraken will then remove ' +
    ...
    function _refreshHomeLumacoreNotice(force) {
        ...
        Bridge.callWithCallback('lumacore_check_update', force ? 'force' : '', function(json) {
            ...
            if (!installed) {
                ...
                if (bodyEl) bodyEl.textContent = 'Install LumaCore so Steam can see games added through Kraken.';
    ...
    function _initToolsGenerators() {
        var gbeBrowse = document.getElementById('tool-gbe-browse');
        if (gbeBrowse) {
            gbeBrowse.addEventListener('click', function() {
                Bridge.callWithCallback('browse_game_folder', function(path) {
                    if (path) document.getElementById('tool-gbe-outdir').value = path;
                });
            });
        }
        var gbeGen = document.getElementById('tool-gbe-generate');
        if (gbeGen) {
            gbeGen.addEventListener('click', function() {
                var cfg = JSON.stringify({
                    api_key: (document.getElementById('tool-gbe-apikey') || {}).value || '',
                    app_id: (document.getElementById('tool-gbe-appid') || {}).value || '',
                    out_dir: (document.getElementById('tool-gbe-outdir') || {}).value || ''
                });
                Bridge.call('generate_gbe_token', cfg);
            });
        }
        ...
        if (vdfBrowse) {
            vdfBrowse.addEventListener('click', function() {
                Bridge.callWithCallback('open_file_dialog', function(path) {
                    if (path) document.getElementById('tool-vdf-path').value = path;
                });
            });
        }
        ...
        if (vdfExtract) {
            vdfExtract.addEventListener('click', function() {
                var path = (document.getElementById('tool-vdf-path') || {}).value || '';
                Bridge.callWithCallback('extract_vdf_keys', path, function(json) {
                    var rows = [];
                    try { rows = JSON.parse(json || '[]'); } catch (e) {}
                    var table = document.getElementById('tool-vdf-table');
                    ...
                    (Array.isArray(rows) ? rows : []).forEach(function(row) {
                        var tr = document.createElement('tr');
                        var cells = [
                            row.app_id || row.appid || '',
                            row.depot_id || row.depotid || '',
                            row.key || row.decryption_key || ''
                        ];
                    ...
                    if (table) table.classList.toggle('hidden', !tbody.children.length);
                    if (!tbody.children.length) Components.showToast('info', 'No depot keys found.');
                });
            });
        }
    }

    function _initSteamStatusChip() {
        var chip = document.getElementById('steam-status-chip');
        if (chip) {
        ...
        function refresh() {
            Bridge.callWithCallback('get_steam_client_status', function(json) {
                var data = {};
                try { data = JSON.parse(json || '{}'); } catch (e) {}
                var el = document.getElementById('steam-status-chip');
                var label = document.getElementById('steam-status-label');
                if (!el) return;
                el.classList.remove('state-running', 'state-stopped', 'state-missing');
                el.classList.add('state-' + (data.state || 'missing'));
                if (label) label.textContent = data.label || 'Steam';
            });
        }
        Bridge.onReady(refresh);
        setInterval(refresh, 20000);
        window._krakenCheckDisk = function(size) {
            var need = Number(size) || 0;
            if (need < 50 * 1024 * 1024) return;
            Bridge.callWithCallback('get_steam_client_status', function(json) {
                var st = {};
                try { st = JSON.parse(json || '{}'); } catch (e) {}
                if (!st.path) return;
                Bridge.callWithCallback('get_disk_usage', st.path, function(djson) {
                    var d = {};
                    try { d = JSON.parse(djson || '{}'); } catch (e) {}
                    if (!d.free) return;
                    if (d.free < need * 1.1) {
                        var freeGb = (d.free / 1073741824).toFixed(1);
                        Components.showToast('warning', 'This download may not fit on the Steam drive (' + freeGb + ' GB free).');
                    }
                });
            });
        };
    ...
    function _initGameCardDetails() {
        document.addEventListener('click', function(e) {
            if (e.target.closest('.btn-download, [data-action], button, a, input, select, label')) return;
            if (window.Store && Store.isSelectMode && Store.isSelectMode()) return;
            var card = e.target.closest('#store-grid .game-card, #store-list .game-list-item, #store-list .steam-row, #library-grid .game-card');
            if (!card || !card.dataset.appid || !window.GameDetails) return;
            var nameEl = card.querySelector('.game-card-name, .game-list-name, .steam-row-name');
            GameDetails.show(card.dataset.appid, nameEl ? nameEl.textContent : '');
        });
        document.addEventListener('keydown', function(e) {
