...
class SFFMainWindow(QMainWindow):
        ...
        try:
            ...
            if failed:
                try:
                    ...
                    QMessageBox.warning(self, "Kraken update — issue", msg)
                except Exception:
                    pass
        except Exception:
    ...
    def _install_web_splash(self):
        ...
        for candidate in ("kraken.png", "kraken.ico", "SFF.png", "SFF.ico"):
            try:
    ...
    def _on_web_view_load_finished(self, ok: bool):
        if not ok:
            self._web_view.setHtml(
                ...
                "<div><h2 style='color:#c9b6ff'>Kraken UI failed to load</h2>"
    ...
    def _set_theme(self, key, save=True):
        key = key if key in THEMES else "steam"
        ...
        if s == Settings.PLAY_MUSIC:
            ...
            if parent_widget:
                QMessageBox.information(
                    parent_widget,
                    "Restart Recommended",
                    "Steam path changed. Please restart Kraken for all changes to take full effect.",
                )
    ...
    def closeEvent(self, event):
        ...
        if (
            ...
            if not self._tray_hide_notified:
                ...
                self._tray.notify(
                    "Kraken",
                    "Kraken is running in the system tray. Click the ^ arrow near the clock to find it.",
                )
        else:
    ...
    def _show_about(self):
        ...
        QMessageBox.about(
            ...
            "About Kraken",
            f"Kraken\nVersion {VERSION}\n\n"
            "https://github.com/haker146/Kraken/releases",
        )
