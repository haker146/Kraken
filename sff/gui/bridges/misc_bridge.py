...
def _bridge_activate_dlcs(bridge, app_id, dlc_ids_json):
    """Activate selected DLC app IDs on Steam (AppList / LumaCore lua)."""
    parent = str(app_id or "").strip()
    if not parent.isdigit():
        bridge._emit_task_result(
            "activate_dlcs", False, "Invalid app ID", app_id=parent, activated=0
        )
        return
    ...
    def _err(msg):
        bridge._emit_task_result(
            "activate_dlcs", False, str(msg) or "Could not activate DLC.", app_id=parent
        )
