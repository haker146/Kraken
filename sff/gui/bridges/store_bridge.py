...
_STEAM_ASSET_BASE = "https://shared.akamai.steamstatic.com/store_item_assets/"
_STEAM_TAG_NAMES = {
    9: "Strategy",
    19: "Action",
    21: "Adventure",
...
def _asset_url(asset_url_format: str, filename: str, appid: int) -> str:
    name = str(filename or "").strip()
    if not name:
        return ""
    if name.startswith("http://") or name.startswith("https://"):
        return name
    fmt = str(asset_url_format or "").strip()
    if fmt and "${FILENAME}" in fmt:
        return _STEAM_ASSET_BASE + fmt.replace("${FILENAME}", name)
    if name.startswith("steam/"):
        return _STEAM_ASSET_BASE + name
    return f"{_STEAM_ASSET_BASE}steam/apps/{appid}/{name}"


def _unix_release_label(epoch) -> str:
    try:
    ...
    if value <= 0:
        return ""
    try:
        return _time.strftime("%d %b %Y", _time.gmtime(value)).lstrip("0")
    except Exception:
        return ""


def _fetch_steam_catalog_items(app_ids):
    """Batch Steam store-row metadata for the in-app catalog list."""
    if not app_ids:
        return {}
    ids = []
    seen = set()
    ...
    for start in range(0, len(ids), chunk_size):
        ...
        try:
            ...
            for item in (data.get("response") or {}).get("store_items", []) or []:
                ...
                header = _asset_url(fmt, assets.get("header") or "header.jpg", aid)
                capsule = _asset_url(
                    fmt,
                    assets.get("main_capsule") or assets.get("small_capsule") or "",
                    aid,
                ) or header
                basic = item.get("basic_info") or {}
                developers = [
                    str(d.get("name"))
                    for d in (basic.get("developers") or [])
                    if isinstance(d, dict) and d.get("name")
                ]
                publishers = [
                    str(p.get("name"))
                    for p in (basic.get("publishers") or [])
                ...
                for shot in shots[:4]:
                    ...
                    url = _asset_url("", filename, aid) if filename else ""
                    if url:
                        thumbs.append(url)
                tag_names = []
                for tag in item.get("tags") or []:
                    if not isinstance(tag, dict):
                        continue
                    tagid = tag.get("tagid")
                    name = _STEAM_TAG_NAMES.get(tagid)
                    if name and name not in tag_names:
                        tag_names.append(name)
                short = str(basic.get("short_description") or "").replace("\r\n", " ").strip()
                if len(short) > 420:
                ...
                out[aid] = {
                    ...
                    "release_date": _unix_release_label(release.get("steam_release_date")),
                    "review_label": str(reviews.get("review_score_label") or ""),
                    "review_percent": int(reviews.get("percent_positive") or 0),
                    "review_count": int(reviews.get("review_count") or 0),
                    "header_image": header,
                    "capsule_url": capsule,
                    "screenshot_thumbs": thumbs,
                    "catalog_tags": tag_names,
                }
        except Exception as e:
            logger.debug("Steam catalog batch fetch failed: %s", e)
    return out


def _enrich_store_rows(games):
    """Attach Steam list metadata + isitcracked protection/crack status."""
    if not games:
        return games
    app_ids = []
    for game in games:
    ...
    catalog = {}
    try:
        catalog = _fetch_steam_catalog_items(app_ids)
    except Exception as e:
        logger.debug("store catalog enrich failed: %s", e)
    cracks = {}
    try:
        from sff.network.isitcracked import lookup_games
        cracks = lookup_games(games) or {}
    except Exception as e:
        logger.debug("isitcracked enrich failed: %s", e)
    for game in games:
        try:
            aid = int(game.get("app_id") or 0)
        except (TypeError, ValueError):
            aid = 0
        meta = catalog.get(aid) or {}
        if meta.get("short_description") and not game.get("short_description"):
            game["short_description"] = meta["short_description"]
        if meta.get("developers"):
            game["developers"] = meta["developers"]
        if meta.get("publishers"):
            game["publishers"] = meta["publishers"]
        if meta.get("release_date") and not game.get("release_date"):
            game["release_date"] = meta["release_date"]
        if meta.get("review_label"):
            game["review_label"] = meta["review_label"]
            game["review_percent"] = meta.get("review_percent") or 0
            game["review_count"] = meta.get("review_count") or 0
        if meta.get("header_image") and not game.get("image_url"):
            game["image_url"] = meta["header_image"]
        if meta.get("header_image"):
            game["header_image"] = game.get("header_image") or meta["header_image"]
        if meta.get("capsule_url"):
            game["capsule_url"] = meta["capsule_url"]
        if meta.get("screenshot_thumbs"):
            game["screenshot_thumbs"] = meta["screenshot_thumbs"]
        tags = game.get("tags") or []
        if not tags and meta.get("catalog_tags"):
            game["tags"] = meta["catalog_tags"]
        crack = cracks.get(aid) or {}
        status = str(crack.get("status") or "").strip()
        protection = str(crack.get("protection") or "").strip()
        if not protection:
            drm = game.get("drm")
            if drm is True:
                protection = "DRM"
            elif isinstance(drm, (list, tuple)) and drm:
                protection = ", ".join(str(x) for x in drm if x)
            elif isinstance(drm, str) and drm.strip() and drm.strip().lower() not in ("false", "0"):
                protection = drm.strip()
        if not status and game.get("crack_buildid"):
            status = "Cracked"
        if status:
...
def _finalize_store_result(result, request_id=''):
    if not isinstance(result, dict):
        return result
    if str(request_id or "").startswith(("home-", "store-suggest-", "store-prefetch-")):
        return result
    try:
        _enrich_store_rows(result.get("games") or [])
    except Exception as e:
        logger.debug("store row enrich failed: %s", e)
    return result
...
def _bridge_search_games(bridge, query, offset, per_page, sort_by='updated', tag='', request_id=''):
    ...
    def _do():
        ...
        if tag and not query:
            result = search_games_by_tag(tag, 0, min(80, max(50, int(per_page or 20) * 3)))
            ...
            return _finalize_store_result(result, request_id)
...
def _bridge_get_store_game_details(bridge, app_id):
    ...
    if not app_id.isdigit():
        bridge._emit_task_result("game_details", False, "Invalid App ID.", app_id=app_id)
        return
    ...
    def _do():
        ...
        dlc_rows = []
        for dlc_id in dlc_ids[:40]:
            name = get_dlc_name(dlc_id) or f"DLC {dlc_id}"
            dlc_rows.append({"id": int(dlc_id), "name": name})

        info = get_game_info(int(app_id)) or {}
        ...
        try:
            ...
            if bridge._steam_path:
                for lib in get_steam_libs(bridge._steam_path):
                    ...
                    if acf.is_file():
                        state = vdf_load(acf).get("AppState") or {}
                        installed_buildid = str(state.get("buildid") or "")
                        installed = True
                        break
        except Exception:
            pass
        ...
        payload = {
            ...
            "genres": details.get("genres") or info.get("tags") or [],
    ...
    bridge._run_async(_do, on_done=_ok, on_error=_err)


def _bridge_get_steam_client_status(bridge):
    from sff.core.processes import is_proc_running
    path = str(bridge._steam_path) if bridge._steam_path else ""
    found = bool(path and Path(path).exists())
    running = False
    try:
        running = is_proc_running("steam.exe") or is_proc_running("steam")
    except Exception:
        running = False
    if found and not running:
        state = "stopped"
        label = "Steam found"
    elif found and running:
        state = "running"
        label = "Steam running"
    else:
        state = "missing"
...
def _bridge_get_catalog_status(bridge):
    from sff.game_list_fallback import catalog_status
    return json.dumps(catalog_status())


def _bridge_get_kraken_favorites(bridge):
    from sff.core.storage.settings import get_setting
    from sff.core.structs import Settings
    return json.dumps(_json_id_list(get_setting(Settings.KRAKEN_FAVORITES) or "[]"))


def _bridge_set_kraken_favorite(bridge, app_id, enabled):
    from sff.core.storage.settings import get_setting, set_setting
    from sff.core.structs import Settings
    app_id = str(app_id or "").strip()
    if not app_id.isdigit():
        return "[]"
    ids = _json_id_list(get_setting(Settings.KRAKEN_FAVORITES) or "[]")
    if enabled:
        if app_id not in ids:
            ids.insert(0, app_id)
    else:
        ids = [x for x in ids if x != app_id]
    set_setting(Settings.KRAKEN_FAVORITES, json.dumps(ids[:80]))
    return json.dumps(ids)


def _bridge_get_kraken_recent(bridge):
    from sff.core.storage.settings import get_setting
    from sff.core.structs import Settings
    return json.dumps(_json_id_list(get_setting(Settings.KRAKEN_RECENT) or "[]"))


def _bridge_add_kraken_recent(bridge, app_id):
    from sff.core.storage.settings import get_setting, set_setting
    from sff.core.structs import Settings
    app_id = str(app_id or "").strip()
    if not app_id.isdigit():
        return "[]"
    ids = [x for x in _json_id_list(get_setting(Settings.KRAKEN_RECENT) or "[]") if x != app_id]
    ids.insert(0, app_id)
    set_setting(Settings.KRAKEN_RECENT, json.dumps(ids[:24]))
    return json.dumps(ids)


def _bridge_get_whats_new_seen(bridge):
    from sff.core.storage.settings import get_setting
    from sff.core.structs import Settings
    val = get_setting(Settings.KRAKEN_WHATS_NEW_1_0)
    return bool(val)


def _bridge_set_whats_new_seen(bridge):
    from sff.core.storage.settings import set_setting
    from sff.core.structs import Settings
