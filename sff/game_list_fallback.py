...
def catalog_status() -> dict:
    """Age and load state of the local games.json catalog."""
    loaded = bool(_games_cache)
    age = None
    if _games_cache_time:
        age = max(0.0, time.time() - float(_games_cache_time))
    return {
        "loaded": loaded,
        "count": len(_games_cache) if loaded else 0,
        "age_seconds": age,
        "stale": (not loaded) or (age is None) or (age > 86400 * 3),
    }
...
def search_games_by_tag(tag: str, offset=0, per_page=20):
    ...
    for appid_str, info in _games_cache.items():
        ...
        if any(t.lower() == tag_lower for t in game_tags):
            ...
            if len(results) >= _TAG_SEARCH_CAP:
                break
