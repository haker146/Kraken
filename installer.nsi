...
!endif
!define EXENAME    "Kraken_GUI.exe"
!define PUBLISHER  "haker146"

Name "${APPNAME}"
OutFile "Kraken-${VERSION}-Setup.exe"
...
!define MUI_ICON    "kraken.ico"
!define MUI_UNICON  "kraken.ico"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://github.com/haker146/Kraken"
...
; ============================================================
Section "Kraken (required)" SEC_MAIN
    SectionIn RO

    SetOutPath "$INSTDIR"
    File /r "dist\Kraken_GUI\*"

    ; Bundle the icon at root so shortcuts and ARP show it
    File /oname=Kraken.ico "kraken.ico"
    File /oname=kraken.png "kraken.png"
    ...
    WriteRegStr  HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\${APPNAME}" "DisplayIcon"      "$INSTDIR\Kraken.ico"
...
Section ".NET 9 Runtime (required for downloads)" SEC_DOTNET
    ...
    check_user_dotnet:
        ; Per-user install at %LOCALAPPDATA%\Microsoft\dotnet — Kraken
    ...
    dotnet_dl_failed:
        ...
        DetailPrint "Skipping. Kraken will offer to install it again on first launch."
        Goto dotnet_end

    dotnet_install_failed:
        DetailPrint ".NET 9 Runtime installer returned exit code $0 (skipping)."
        DetailPrint "Kraken will offer to install it again on first launch."
...
Section "Desktop Shortcut" SEC_DESKTOP
    CreateShortcut "$DESKTOP\${APPNAME}.lnk" "$INSTDIR\${EXENAME}" "" "$INSTDIR\Kraken.ico"
SectionEnd
...
Section "Start Menu Shortcut" SEC_STARTMENU
    ...
    CreateShortcut  "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk"  "$INSTDIR\${EXENAME}" "" "$INSTDIR\Kraken.ico"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    ...
    !insertmacro MUI_DESCRIPTION_TEXT ${SEC_VCREDIST}  "Visual C++ runtime libraries used by Kraken and bundled tools. Skipped automatically if already present."
