...
try:
    ...
    _msg = (
        "Kraken failed to start because PyQt6 is missing from the install.\n\n"
        ...
        "https://github.com/haker146/Kraken/releases/latest\n\n"
    ...
    if sys.platform == "win32":
        try:
            import ctypes
            ctypes.windll.user32.MessageBoxW(0, _msg, "Kraken — incomplete install", 0x10)
        except Exception:
            pass
    else:
        try:
...
try:
    ...
    QMessageBox.critical(None, "Kraken startup error", msg[:2000])
    sys.exit(1)
...
def main():
    ...
    _app_icon = QIcon()
    _icon_candidates = list(("kraken.ico", "kraken.png", "SFF.ico", "sff.ico", "SFF.png", "sff.png"))
    ...
    if sys.platform == "linux":
        ...
        if _appdir:
            _icon_candidates.insert(0, os.path.join(_appdir, "kraken.png"))
            _icon_candidates.insert(0, os.path.join(_appdir, "Kraken.png"))
    ...
    if sys.platform == "linux":
        ...
        if _appimage:
            try:
                ...
                _icon_dest = _icon_dest_dir / "Kraken.png"
                _desktop_dir = _home / ".local/share/applications"
                _desktop_file = _desktop_dir / "kraken.desktop"
                _appdir_env = os.environ.get("APPDIR", "")
                _icon_src = None
                if _appdir_env:
                    for _iname in ("Kraken.png", "kraken.png"):
                        _cand = Path(_appdir_env) / _iname
                        if _cand.exists():
                            _icon_src = _cand
                            break
                ...
                if _needs_write:
                    ...
                    _desktop_file.write_text(
                        ...
                        "Version=1.0\n"
                        "Name=Kraken\n"
                        "Comment=Steam catalog front end\n"
                        f"{_new_exec}\n"
                        "Icon=Kraken\n"
                        "Terminal=false\n"
                        "Type=Application\n"
    ...
    if _tray_menu is not None:
        _quit_action = _QAction("Quit Kraken", _tray_menu)
